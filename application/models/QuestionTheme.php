...
class QuestionTheme extends LSActiveRecord
{
    ...
    public static function findAllQuestionMetaDataForSelector()
    {
        ...
        foreach ($baseQuestions as $baseQuestion) {
            ...
            $filename = App()->getConfig('rootdir') . DIRECTORY_SEPARATOR . $baseQuestion['xml_path'] . DIRECTORY_SEPARATOR . 'config.xml';
            if (!file_exists($filename)) {
                App()->setFlashMessage(
                    sprintf(
                        gT('Did not find config.xml for question theme %s'),
                        $baseQuestion['title']
                    ),
                    'error'
                );
            ...
            $sQuestionConfigFile = file_get_contents($filename);
            $oQuestionConfig = simplexml_load_string($sQuestionConfigFile);
